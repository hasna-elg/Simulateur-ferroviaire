...
const VITESSE = 500; // 500ms = 0.5s pour deux coups par seconde
...
class Train {
    constructor(x, y, direction, wagons) {
        this.x = x;
        this.y = y;
    ...
    avancer(plateau) {
        ...
        const casesuivante = plateau.cases[suivantX][suivantY];
        if (this.case_incompatible(casesuivante)) {
            return false; 
            // case incompatible
        }
...
class Simulateur {
    constructor(context) {
        ...
        this.enPause = false; 
        this.initvent(); // Initialisation des événements
        this.commecesimulation(); // Commencer la simulation
    }
    ...
    mettre_a_jour() {
        // Avancer chaque train
        if (!this.enPause) {
            for (let i = this.trains.length - 1; i >= 0; i--) {
                const train = this.trains[i];
                if (!train.avancer(this.plateau)) {
                    this.trains.splice(i, 1); // Retirer le train s'il sort du plateau ou rencontre une case incompatible
                }
            }
            this.dessiner();
        }
    ...
    initvent() {
        ...
        document.getElementById('simulateur').addEventListener('click', (event) => {
            ...
            if (this.selectedTrain !== null) {
                this.ajouter_train(x, y, this.selectedTrain);
            }
        });
    ...
    ajouter_train(x, y, wagonsCount) {
        ...
        const train = new Train(x, y, 'droite', wagons);
...
function tchou() {
    ...
    let simulateur = new Simulateur(contexte);
    simulateur.demarrer();
}
